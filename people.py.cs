...
public static class people {
    ...
    public class player {
        ...
        public player(string name, object location, skills skills, magic magic) {
            this.name = name;
            this.location = location;
            this.skills = skills;
            this.magic = magic;
    ...
    public static void createPlayer(object race, object loc) {
        // for i in range(len(inv[0])):
        //    inv[0][i]=it.createItem(inv[0][i])
        g.setText(label4: "Enter your name:");
        g.gwin.button0["text"] = "Confirm";
        g.gwin.button0["command"] = () => g.setName(g.gwin.textInput.get());
        var skill = new skills();
        var mag = new magic();
        if (race == "human") {
            me = new player("", loc, skill, mag);
        }
