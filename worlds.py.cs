...
public static class worlds {
    ...
    public class shoppingBag {
        ...
        public shoppingBag(object item, int holding, int wants) {
            this.item = item;
    ...
    public class harvest {
        ...
        public harvest(int jobID, void worker, object homeShop, object item) {
            this.jobID = jobID;
            this.worker = worker;
            this.homeShop = homeShop;
            this.item = item;
    ...
    public static void buildWorld(object numCities, object infestation) {
        object tempsite;
        ...
        var web = nx.balanced_tree(4, 3);
        web.add_edge(1, 2);
        web.add_edge(2, 3);
        web.add_edge(3, 4);
        web.add_edge(4, 1);
        var color_map = new List<string> {
            "red"
        } * 85;
        var terraincolors = new List<string> {
            "green",
            "purple",
            "lime",
            "brown",
        ...
        foreach (var node in Enumerable.Range(0, 21 - 0)) {
            if (node == 0) {
                color_map[node] = "green";
                web.nodes[node]["terrain"] = "grassland";
            } else if (Enumerable.Range(1, 5 - 1).Contains(node)) {
                color_map[node] = "tan";
                web.nodes[node]["terrain"] = "grassland";
            } else if (Enumerable.Range(5, 21 - 5).Contains(node)) {
                // TODO create country
                var randterrain = r.randrange(0, terraincolors.Count);
                color_map[node] = terraincolors[randterrain];
                web.nodes[node]["terrain"] = terraintypes[randterrain];
                foreach (var i in Enumerable.Range(1, 5 - 1)) {
